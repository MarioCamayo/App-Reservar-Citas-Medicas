...
export const MyAppointments = () => {
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-700 border-b'>Mis Citas </p>
      <div>
       {
            ...
            className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b'
             key={index}>
              <div>
                ...
                src= { item.image } alt="imagen de doctores" />
              </div>
              <div className='flex-1 text-sm text-zinc-600'>
                <p className='text-neutral-800 font-semibold'>{ item.name }</p>
                <p>{ item.speciality }</p>
                <p className='text-zinc-700 font-medium mt-1'>Direción:</p>
                <p className='text-xs'>{ item.address.line1 }</p>
                <p className='text-xs'>{ item.address.line2 }</p>
                <p className='text-xs mt-1'>
                  <span className='text-sm text-neutral-700 font-medium '>Fecha y hora:</span>
                  25, Julio, 2025 | 8:30 PM
                </p>  
              </div>
              <div>
